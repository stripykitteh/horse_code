...
csv = '/Users/phillipmonk/research_paper/data/meetings.csv'
...
for (let meet_no = arg[0]; meet_no < arg[1]; meet_no++) {
	...
	meet_stub = track_arr[1].substring(0,4) + '-' + track_arr[1].substring(4,6) + '-' + track_arr[1].substring(6,8) + '/' + track_arr[0];
...
//	await page.screenshot({ path: '/Users/phillipmonk/research_paper/screenshots/racing_com_form_' + meet_stub_under + '.png' });
	...
	fs.writeFile('/Users/phillipmonk/research_paper/html/racing_com_form_' + meet_stub_under + '.html', html, err => {
	    if (err) {
		console.error(err);
	    }
