...
csv = '/Users/phillipmonk/research_paper/data/races_sorted_racenet.csv'
var races = fs.readFileSync(csv)
    .toString()
    .split('\n')
    .map(e => e.trim())

page_root = 'https://racenet.com.au/results/horse-racing/';
...
//	const browser = await chromium.launch({headless: true}) //false, args:['--start-maximized']});
	...
	race_stub = race_arr[1] + '-' + race_arr[0] + '/' + race_arr[3] + '-race-' + race_arr[2];
	...
	fs.writeFile('/Users/phillipmonk/research_paper/html/races_racenet/racenet_com_au_' + race_stub_under + '.html', html, err => {
	    if (err) {
		console.error(err);
	    }
